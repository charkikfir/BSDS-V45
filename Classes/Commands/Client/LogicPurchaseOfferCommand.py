...
class LogicPurchaseOfferCommand(LogicCommand):
    ...
    def execute(self, calling_instance, fields):
        ...
        if fields["Unk1"] == 0:
            player_data["OwnedBrawlers"][61] = {'CardID': 507, 'Skins': [], 'Trophies': 0, 'HighestTrophies': 0, 'PowerLevel': 1, 'PowerPoints': 0, 'State': 2}
            ...
            a = [61]
            for x in a:
            	item = {'Amount': 1, 'DataRef': [16, 61], 'RewardID': 1}
            	box['Items'].append(item)
            player_data["delivery_items"]['Boxes'].append(box)

        if fields["Unk1"] == 1:
            player_data["OwnedBrawlers"][60] = {'CardID': 499, 'Skins': [], 'Trophies': 0, 'HighestTrophies': 0, 'PowerLevel': 1, 'PowerPoints': 0, 'State': 2}
            player_data["PushasedOffers"].append(-3)
            player_data["delivery_items"] = {
                'Boxes': []
            }
            ...
            a = [60]
            for x in a:
            	item = {'Amount': 1, 'DataRef': [16, 60], 'RewardID': 1}
            	box['Items'].append(item)
            player_data["delivery_items"]['Boxes'].append(box)
