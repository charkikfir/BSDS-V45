...
class AllianceDataMessage(PiranhaMessage):
    ...
    def encode(self, fields, player):
        ...
        for i in clubdb_instance.getMembersSorted(clubData):
            ...
            self.writeVInt(0) # Season Club Trophies
            self.writeVInt(0) # Day Club Trophies
            self.writeVInt(0) # Season Tickets Used
            self.writeVInt(0) # Day Tickets Used
            self.writeVInt(0) # Day Tickets Usex Max
            self.writeVInt(0) # Idk
            self.writeBoolean(True)
